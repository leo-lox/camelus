...
class AddMintNotifier extends StateNotifier<ValidationState> {
  ...
  void updateText(String text) {
    ...
    if (text.isEmpty) {
      state = state.copyWith(
          isValidating: false, isValid: null, clearMintInfo: true);
      return;
    }
  ...
  Future<void> _validateText(String text) async {
    ...
    try {
      final mintInfoNetwork =
          await _ndk.cashu.getMintInfoNetwork(mintUrl: buildValidUrl(text));
...
final addMintProvider =
    StateNotifierProvider.autoDispose<AddMintNotifier, ValidationState>(
  (ref) {
            ...
            if (validationState.mintInfo != null)
              SingleChildScrollView(
                child: MintInfoCardSmall(
                  mintInfo: validationState.mintInfo!,
                ),
              ),
          ...
          child: longButton(
            ...
            onPressed: () {
              if (validationState.isValid == true &&
                ...
                ref.read(ndkProvider).cashu.addMintToKnownMints(
                      mintUrl: validUrl,
                    );
